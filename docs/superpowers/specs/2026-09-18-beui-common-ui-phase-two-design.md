...
대시보드의 반투명 톤은 의도된 디자인이므로 유지합니다. 카드·패널마다 흩어진
배경 투명도·테두리·그림자 값을 같은 파일의 토큰으로 모읍니다.

```
--surface-translucent: <기존 대시보드 카드 배경값>
--surface-translucent-strong: <기존 사이드 메뉴/표 배경값>
--surface-border-translucent: <기존 얇은 전체 테두리>
--surface-shadow-soft: <기존 부드러운 그림자>
```

값은 현재 `page.scss`·`side-menu.scss`에 있는 실제 값을 그대로 옮깁니다.
`backdrop-filter`는 다시 넣지 않습니다 — 2026-09-14 기록대로 창 크기 변경 중
합성 레이어 깜빡임 때문에 제거한 상태입니다. 다크 모드 값도 같은 방식으로
함께 옮깁니다.
...
대시보드 카드·패널의 표면을 `Card.tsx` 하나로 모읍니다. 반투명 톤은 유지하며
A-2의 토큰을 사용합니다. 패딩·테두리 반경·그림자를 한 곳에서 정의해 카드마다
다른 값이 생기지 않게 합니다.

- props: `tone`(기본/강조), `padding`(기본/컴팩트), `as`.
- 개요 카드의 미세한 색감 차이는 2026-09-14 결정대로 유지합니다.
