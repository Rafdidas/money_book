...
const loadStockMaster = async () => {
  ...
  stockMasterPromise = Promise.all(
    ...
    itemsByMarket.flat().map((item) => ({
      ...
      compactLatinName: normalizeStockSearchText(item.name).replace(/[^a-z0-9]/g, ""),
    })),
  );
...
export const searchKisStocks = async (query: string) => {
  ...
  return entries
    .map((entry) => {
      const { compactLatinName, normalizedName, normalizedSymbol } = entry;
      const latinBrand = normalizedName.match(/^[a-z]+/)?.[0] ?? "";
      ...
      if (normalizedSymbol === keyword) score = 0;
        ...
        keyword.length > latinBrand.length &&
        compactLatinName.startsWith(keyword) &&
        /\d/.test(compactLatinName)
      ) {
        score = 2.5;
      }
      else if (normalizedName.startsWith(keyword)) score = 3;
      else if (compactLatinName.startsWith(keyword)) {
        score = /\d/.test(compactLatinName) ? 4 : 4.5;
      }
      else if (normalizedSymbol.includes(keyword)) score = 5;
      else if (normalizedName.includes(keyword)) score = 6;
      else if (compactLatinName.includes(keyword)) score = 7;
