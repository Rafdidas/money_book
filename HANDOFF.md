...
- 검증: 구현 전 대상 테스트가 실패하고 구현 후 통과했으며, `npm test`(44개 파일·189개 테스트), `npm run lint`, `npm run build`, `git diff --check`를 통과했습니다. 데모 모드에서 데스크톱(1280×900)·모바일(390×844)로 만기 처리 모달과 `납입 안 함` 선택 후 저축 50만 원 집계가 사라지는 흐름을 확인했습니다.
