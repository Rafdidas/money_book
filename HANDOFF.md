# 2026-08-25 사용자 카테고리 관리 설계

- 기존 `user_custom_categories`를 확장해 기본 카테고리를 보존하면서 사용자 카테고리 추가·이름 수정·삭제를 지원하는 설계를 확정했습니다.
- 사용자 카테고리 중 유형별 최대 5개를 `자주 쓰는 카테고리`로 지정하고 거래 입력 화면에서 빠른 선택 칩으로 사용합니다.
- 공통 관리 컴포넌트를 거래 입력 모달과 마이페이지 카드에서 재사용하며, 데모 모드도 같은 기능을 제공합니다.
- 사용자 카테고리를 수정·삭제해도 기존 거래 내역의 카테고리 문자열은 변경하지 않습니다.
- 설계 문서: `docs/superpowers/specs/2026-08-25-custom-category-management-design.md`
- 구현 계획 작성 및 사용자 설계 승인 완료: `docs/superpowers/plans/2026-08-25-custom-category-management.md`
- 현재 단계: 코드 구현 직전. 구현 모델로 변경한 뒤 TDD 순서로 Task 1부터 실행해야 합니다.
- 프로덕션 DB/데이터 변경: 없음. 구현 시 `is_favorite` 및 유형별 최대 5개 제한 마이그레이션이 필요합니다.
