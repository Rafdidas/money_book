...
- 핵심 가치, 주요 기능, 데모·데이터 분리·RLS 안내, 기술 구성, 로컬 실행과 데이터베이스 설정을 현재 코드 기준으로 정리했습니다.
- 검증: README의 로컬 이미지 경로 존재 여부와 주요 섹션·링크를 확인했고 `git diff --check`를 통과했습니다.

# 2026-09-04 적금 만기 월 납입 선택

- 적금 만기 처리 시 선택한 월의 납입을 `납입함` 또는 `납입 안 함`으로 고를 수 있는 모달을 추가했습니다.
- `납입 안 함`을 고르면 선택 월의 예정 납입과 이후 예정 납입을 취소하고, `납입함`은 선택 월 납입을 유지한 채 이후 납입만 취소합니다.
- 완료 상태는 저장된 적금 계정의 `status`를 기준으로 표시해, 만기 월에도 처리 전에는 선택 모달을 열 수 있습니다.
- 이미 완료된 저장형 적금에는 `만기 월 납입 수정` 버튼을 표시해, 메뉴를 찾지 않아도 같은 선택 모달을 다시 열 수 있습니다.
- 기존 거래 기반 적금도 만기 월이면 같은 `만기 월 납입 수정` 버튼을 표시하도록 보완했습니다.
- 회귀 테스트: 완료 상태 적금의 만기 월 납입 수정 가능 여부를 검증했습니다. 구현 전 실패, 구현 후 통과했습니다.
- 회귀 테스트: `src/app/_home/utils.test.ts`에 만기 월 납입 포함·제외 분기 테스트를 추가했습니다. 구현 전 두 분기가 실패했고, 구현 후 통과했습니다.
- 검증: 구현 전 대상 테스트가 실패하고 구현 후 통과했으며, `npm test`(44개 파일·190개 테스트), `npm run lint`, `npm run build`, `git diff --check`를 통과했습니다. 데모 모드에서 데스크톱(1280×900)·모바일(390×844)로 만기 처리 모달과 `납입 안 함` 선택 후 저축 50만 원 집계가 사라지는 흐름을 확인했습니다.
...
# 2026-09-03 로그인 진행 피드백 및 중복 인증 조회 개선

- 원인: 로그인 핸들러의 `finally`가 화면 이동 전에 제출 상태를 해제했습니다. 인증 성공 직후 사용자 재조회 1회, 앱 진입 시 사용자 재조회 2회가 순차 실행됐으며 `replace` 직후 `refresh`도 호출했습니다.
- 로그인 상태를 인증 중/화면 이동 중으로 구분하고 CSS 회전 표시·상태 문구를 추가했습니다. 8초 이상 걸리면 대기 안내를 표시하며 성공 후에는 화면이 바뀔 때까지 중복 제출을 막습니다. 실패 시 입력값을 보존하고 폼 안에서 한국어 오류를 안내합니다.
- 로그인 응답의 검증된 사용자를 이동 경로 판단에 재사용하고 앱 초기화도 사용자 조회 1회로 통합했습니다. 약관 동의 확인과 서버 proxy 인증은 유지합니다. 로그인/데모 진입 시 불필요한 `refresh`를 제거했습니다.
- 초기 세션 확인이 늦게 끝나 진행 중인 로그인과 경합하는 경우를 방지했습니다.
